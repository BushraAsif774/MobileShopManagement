            ...
            <div class="row">
                <div class="col-sm-9">
                    <?php
                        ...
                        //here foreach return all data in the card table
                        //and get data in item variable one by one
                        foreach ($product->getData('cart') as $item) :
                        $cart=$product->getProduct($item['item_id']);
                        //print_r($item);// use  to display array with item id
                        //print_r($cart);//use to display product info using itemid

                        $subTotal[]=array_map(function($item){
                        //The array_map() function sends each value of an array to a user-made
                        // function, and returns an array with new values, given by the user-made function.
                        //Syntax: array_map(function, array)
                    
                    ?>
                    <!-- cart item -->
                        <div class="row border-top py-3 mb-3 mt-3">
                            <div class="col-sm-2 ">
                                <img src="<?php echo $item['item_image'] ?? "./assests/products/1.png" ?>" style="height: 100px;" alt="cart1" class="img-fluid">
                            </div>
                            <div class="col-sm-8 ">
                                ...
                                <!-- product qty -->
                                    <div class="qty d-flex pt-2">
                                        <div class="d-flex font-rale w-25">
                                            ...
                                            <input type="text" data-id="<?php echo $item['item_id'] ?? 0 ?>" class="qty_input border px-3 w-100 h-100 bg-light text-dark" disabled value="1" placeholder="1">
                                            <button data-id="<?php echo $item['item_id'] ?? 0 ?>" class="qty-down border bg-light"><i class="fas fa-angle-down"></i></button>
                                        </div>

                                        <!-- grab these button into form to get item_id for deleting and save later -->
                    ...
                    <?php
                        ...
                        endforeach; //end of for each loop
                        //print_r($subTotal); //display subtotal
                    ?>
